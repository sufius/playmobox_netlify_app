...
> 🚨 If you decide to change the project name be sure to change it everywhere in the project including the [Netlify configuration file, `netlify.toml`](./netlify.toml), as there are many places in Angular projects where the project name is used. A quick fix is to find/replace all instances of `playmobox` with your project name.
...
- Use the Netlify CLI's create from template command `netlify sites:create-template playmobox` which will create a repo, Netlify project, and deploy it
